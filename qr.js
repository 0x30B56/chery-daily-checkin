const $ = new Env("奇瑞汽车");
 const _key = 'chery_data';
 $.ck = $.toObj(getEnv(_key)) || {
}
;
 $.is_debug = 'true-';
 $.messages = [];
 ...
 async function tasks() {
  url = `https://mobile-consumer.chery.cn/web/task/record/sign-in/lottery?taskCode=SignUpLottery03&access_token=${
  $.ck.token
 }
 `;
  headers = {
   "Host": "mobile-consumer-sapp.chery.cn", "Connection": "keep-alive", "Authorization": "Bearer " + cookie, "accept-language": "zh-CN,zh", "User-Agent": "Mozilla/5.0 (Linux;
   Android 10;
   MI 8 Lite Build/QKQ1.190910.002;
   wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/81.0.4044.138 Mobile Safari/537.36 android/1.0.0", "content-type": "application/json", "Accept": "*
  /*", "Origin": "https://hybrid-sapp.chery.cn", "X-Requested-With": "com.digitalmall.chery", "Sec-Fetch-Site": "same-site", "Sec-Fetch-Mode": "cors", "Sec-Fetch-Dest": "empty", "Referer": "https://hybrid-sapp.chery.cn", "Accept-Encoding": "gzip, deflate" 
 }
  const {
  status, message
 }
 ...
 //取ck function getCk() {
  if ($request && $request.method != 'OPTIONS') {
   const token = getParameterByName('access_token', $request.url) const ckVal = $.toStr({
   token
  }
  );
   $.setdata(ckVal, _key) $.msg($.name, '', '获取签到数据成功🎉\n' + ckVal) 
 }
}
 // 脚本执行入口 (async () => {
 ...
 try{
  ...
  const _http=[new Promise((_,reject)=>setTimeout(()=>reject(`??请求超时:${
   options['url']
  }
  `),_timeout)),new Promise((resolve,reject)=>{
...
;
 ...
 return new class{
  ...
  lodash_set(t,e,s){
   return Object(t)!==t?t:(Array.isArray(e)||(e=e.toString().match(/[^.[\]]+/g)||[]),e.slice(0,-1).reduce((t,s,a)=>Object(t[s])===t[s]?t[s]:t[s]=Math.abs(e[a+1])>>0==+e[a+1]?[]:{
   }
   ,t)[e[e.length-1]]=s,t)
  }
  getdata(t){
  ...
  get(t,e=(()=>{
   ...
   switch(t.headers&&(delete t.headers["Content-Type"],delete t.headers["Content-Length"],delete t.headers["content-type"],delete t.headers["content-length"]),t.params&&(t.url+="?"+this.queryStr(t.params)),this.getEnv()){
    ...
    ,Object.assign(t.opts,{
     ...
     const{
      statusCode:s,statusCode:a,headers:r,body:i,bodyBytes:o
     }
     =t;
     e(null,{
      status:s,statusCode:a,headers:r,body:i,bodyBytes:o
     }
     ,i,o)
    }
    ...
    this.initGotEnv(t),this.got(t).on("redirect",(t,e)=>{
     ...
     const{
      statusCode:a,statusCode:r,headers:i,rawBody:o
     }
     =t,n=s.decode(o,this.encoding);
     e(null,{
      status:a,statusCode:r,headers:i,rawBody:o,body:n
     }
     ,n)
    }
    ,t=>{
  ...
  post(t,e=(()=>{
   ...
   switch(t.body&&t.headers&&!t.headers["Content-Type"]&&!t.headers["content-type"]&&(t.headers["content-type"]="application/x-www-form-urlencoded"),t.headers&&(delete t.headers["Content-Length"],delete t.headers["content-length"]),this.getEnv()){
    ...
    ,t=>e(t&&t.error||"UndefinedError"));
    break;
    case"Node.js":let a=require("iconv-lite");
    this.initGotEnv(t);
    const{
     url:r,...i
    }
    ...
    this.got[s](r,i).then(t=>{
     const{
      statusCode:s,statusCode:r,headers:i,rawBody:o
     }
  ...
  queryStr(t){
   ...
   for(const s in t){
    let a=t[s];
    null!=a&&""!==a&&("object"==typeof a&&(a=JSON.stringify(a)),e+=`${
     s
    }
    =${
     a
  ...
  msg(e=t,s="",a="",r){
    ...
    switch(typeof t){
     ...
     case"object":switch(this.getEnv()){
      ...
      case"Quantumult X":{
       ...
       return{
        "open-url":e,"media-url":s,"update-pasteboard":a
       }
      }
      case"Node.js":return
     }
   ...
   if(!this.isMute)switch(this.getEnv()){
    case"Surge":case"Loon":case"Stash":case"Shadowrocket":default:$notification.post(e,s,a,i(r));
    break;
    case"Quantumult X":$notify(e,s,a,i(r));
    break;
    case"Node.js":
   }
   if(!this.isMuteLog){
